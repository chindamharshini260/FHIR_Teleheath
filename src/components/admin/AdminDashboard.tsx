...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ user }) => {
  ...
  const [doctorsList, setDoctorsList] = useState<UserAccount[]>([]);
  ...
  useEffect(() => {
    // Real-time Firestore subscription for Doctors queue
    const unsubDoctors = api.subscribeToDoctors(
      (realtimeDoctors) => {
        setDoctorsList(realtimeDoctors);
        setLoading(false);
      },
      (err) => {
        console.warn('Real-time doctors listener note:', err);
        setLoading(false);
      }
    );

    // Real-time Firestore subscription for all accounts
    const unsubUsers = api.subscribeToAllUsers(
      (realtimeUsers) => {
        setUsers(realtimeUsers);
      },
      (err) => {
        console.warn('Real-time users listener note:', err);
      }
    );

    // Initial audit logs load
    api.getAuditEvents()
      .then(setAuditLogs)
      .catch((err) => console.warn('Audit events load note:', err));
  ...
  const loadAdminData = async () => {
    setLoading(true);
    try {
      const [allUsers, docs, audits] = await Promise.all([
        api.getAllUsers(),
        api.getDoctors(),
        api.getAuditEvents(),
      ]);
  ...
  const handleVerifyDoctor = async (doctorId: string, status: 'APPROVED' | 'REJECTED') => {
    ...
    try {
      ...
      setActionMessage(
        status === 'APPROVED'
          ? 'Doctor account has been APPROVED. The physician can now log into the Doctor Dashboard.'
          : 'Doctor account has been REJECTED / DISABLED. Access to the Doctor Dashboard is blocked.'
      );
      setTimeout(() => setActionMessage(null), 5000);
    } catch (err: any) {
      setActionMessage(`Failed to update doctor verification: ${err.message}`);
    } finally {
      setProcessingId(null);
    }
  };

  // Doctors list comes from real-time Firestore query
  const doctors = doctorsList.length > 0 ? doctorsList : users.filter((u) => u.role === 'doctor');
  const pendingDoctors = doctors.filter((d) => (d.doctorStatus || 'PENDING') === 'PENDING');
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Live clinician credential verification, approval queue, user access control, and FHIR AuditEvent provenance.
            </p>
          </div>
        </div>
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 transition-colors self-start md:self-auto cursor-pointer"
        >
      ...
      {actionMessage && (
        <div className="p-3.5 rounded-xl bg-teal-50 border border-teal-200 text-xs text-teal-900 flex items-center gap-2 animate-in fade-in">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-slate-100">
            <div>
              ...
              <p className="text-xs text-slate-500">
                Doctors registered with status = PENDING require administrator approval before clinical dashboard access is granted. Real-time updates active.
              </p>
            </div>
          ...
          {doctors.length === 0 ? (
            <div className="py-16 text-center bg-slate-50 rounded-xl border border-slate-200">
              ...
              <p className="text-xs text-slate-500 mt-1">Newly registered doctors will appear here automatically in real time.</p>
            </div>
          ) : (
              ...
              <table className="w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-slate-100">
                  {doctors.map((doc) => {
                    ...
                    const isDocPending = (doc.doctorStatus || 'PENDING') === 'PENDING';
                    const isApproved = doc.doctorStatus === 'APPROVED';
                    const isRejected = doc.doctorStatus === 'REJECTED';
                    const isProcessing = processingId === docId;

                    return (
                      <tr key={docId} className="hover:bg-slate-50/60">
                        <td className="py-3 px-4">
                          <span className="font-bold text-slate-900 block">{doc.fullName || doc.name || 'Dr. Physician'}</span>
                        ...
                        <td className="py-3 px-4 text-slate-700">{doc.specialty || 'General Telehealth'}</td>
                          ...
                          {isDocPending ? (
                            <>
                              <button
                                ...
                                onClick={() => handleVerifyDoctor(docId, 'APPROVED')}
                                className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold text-white bg-emerald-700 hover:bg-emerald-800 transition-colors disabled:opacity-50 cursor-pointer"
                              >
                                <UserCheck className="w-3.5 h-3.5" />
                                <span>{isProcessing ? 'Updating...' : 'Approve / Enable'}</span>
                              </button>
                              <button
                                disabled={isProcessing}
                                onClick={() => handleVerifyDoctor(docId, 'REJECTED')}
                                className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold text-rose-700 bg-rose-50 hover:bg-rose-100 border border-rose-200 transition-colors disabled:opacity-50 cursor-pointer"
                              >
                                <UserX className="w-3.5 h-3.5" />
                                <span>Reject / Disable</span>
                              </button>
                            </>
                          ) : isApproved ? (
                            <button
                              disabled={isProcessing}
                              onClick={() => handleVerifyDoctor(docId, 'REJECTED')}
                              className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium text-rose-600 bg-rose-50 hover:bg-rose-100 border border-rose-200 transition-colors cursor-pointer"
                            >
                              <UserX className="w-3 h-3" />
                              <span>Disable / Revoke</span>
                            </button>
                          ) : (
                            <button
                              disabled={isProcessing}
                              onClick={() => handleVerifyDoctor(docId, 'APPROVED')}
                              className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold text-white bg-emerald-700 hover:bg-emerald-800 transition-colors cursor-pointer"
                            >
                              <UserCheck className="w-3 h-3" />
                              <span>Re-Approve / Enable</span>
                            </button>
                          )}
                        </td>
                      </tr>
